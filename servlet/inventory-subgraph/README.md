...
Spring Boot 4.1 / Spring GraphQL Inventory subgraph on **Spring MVC and Tomcat**.
Clients are `RestClient` and `HttpSyncGraphQlClient`, returning plain values.

Its twin is [`reactive/inventory-subgraph`](../../reactive/inventory-subgraph). Both publish the same
...
```sh
docker build -f servlet/inventory-subgraph/Dockerfile .
```

Built from the repository root — there is no wrapper in this directory:

```sh
./mvnw -pl servlet/inventory-subgraph -am verify
```
